...
import ShowRoleDetailModal from './ShowRoleDetailModal';
...
export default function TableRoles({ reloadTrigger, onReload }: { reloadTrigger?: number, onReload?: () => void }){
      ...
      },
      {
        ...
        cell: (row: CellContext<Role, unknown>) => (          
          <div className="flex items-center space-x-2">
            ...
            <Button size="xs" variant="warning" onClick={() => onEditRole(row.row.original)}>
              <BiSolidEditAlt />
            </Button>
            <Button size="xs" variant="danger" onClick={() => onDeleteRole(row.row.original)}>
              <FaRegTrashAlt />
            </Button>
          </div>
        )
